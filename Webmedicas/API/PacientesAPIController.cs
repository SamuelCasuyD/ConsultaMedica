    ...
    public class PacientesAPIController : ApiController
    {
        ...
        public Models.RespuestaDTO PutUpdatePaciente(Models.PacienteDTO value)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("SP_DatosPacientes", connection))
            {
                ...
                cmd.Parameters.AddWithValue("param3", value.FechaCreacion);
                cmd.Parameters.AddWithValue("param4", value.Sexo);
                cmd.Parameters.AddWithValue("param5", value.DPI);
                cmd.Parameters.AddWithValue("param6", value.Eliminado);
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        respuesta.Mensaje = (a == 0) ? "No se pudo crear el paciente" : "Creación exitosa";
                    }
                    catch (Exception ex)
                    {
            ...
            using (SqlCommand cmd = new SqlCommand("SP_DatosPacientes", connection))
            {
                ...
                try
                {
                    ...
                    try
                    {
                        a = cmd.ExecuteNonQuery();
                        respuesta.Resultado = true;
                        respuesta.Mensaje = (a == 0) ? "No se pudo actualizar ningun registro" : "Actualización exitosa";
                    }
                    catch
                    {
            ...
            using (SqlCommand cmd = new SqlCommand("SP_DatosPacientes", connection))
            {
                ...
                try
                {
                    ...
                    try
                    {
                        a = cmd.ExecuteNonQuery();
                        respuesta.Resultado = true;
                        respuesta.Mensaje = (a == 0) ? "No se pudo actualizar ningun registro" : "Eliminación exitosa";
                    }
                    catch
                    {
                        respuesta.Mensaje = "No fue posible realizar la transaccion";
